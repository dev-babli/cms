...
export async function GET(
  ...
  try {
    const item = await whitepapers.getBySlug(params.id);
    if (!item) {
...
export async function PUT(
  ...
  try {
    ...
    const result = await whitepapers.update(parseInt(params.id), validated);
...
export async function DELETE(
  ...
  try {
    await whitepapers.delete(parseInt(params.id));
    return NextResponse.json(
      { success: true },
      { headers: { 'Access-Control-Allow-Origin': '*' } }
    );
